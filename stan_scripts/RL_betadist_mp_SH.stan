...
transformed parameters {
  ...
  b2_sh = b2_sh_m + (b2_sh_s * b2_sh_raw);
  ...
  for (s in 1:nS){
        ...
        beta_2_normal[s,2] = beta_2_m + (beta_2_s*beta_2_raw[s]) + b2_sh[s];
  }
...
model {
 //define variables
  ...
  alpha_sh_m ~ normal(0,5);
  b1MF_sh_m ~ normal(0,5);
  b1MB_sh_m ~ normal(0,5);
  b2_sh_m ~ normal(0,5);
