 ...
 //define variables
  ...
  beta_b=1;
    ...
    for (t in 1:nT) {
      ...
      Q_S1[1] = omega * Q_MB[1] + (1-omega) * Q_TD[1]; 
      Q_S1[2] = omega * Q_MB[2] + (1-omega) * Q_TD[2]; 
      ...
      if (missing_choice[s,t,1]==0) {
        ...
        //print(beta_1_MF[s,t]);
         choice[s,t,1] ~ bernoulli_logit(  (Q_S1[2]-Q_S1[1])*beta_1[s] + (pers[s]*prev_choice) + (Mo_pers[s]*prev_motor););
         }
...
generated quantities {
  ...
  beta_b[s,1]=1;
    for (t in 1:nT) {
      ...
      Q_S1[s,t,1] = omega[s] * Q_MB[s,t,1] + (1-omega[s]) * Q_TD[s,t,1] + (pers[s]*px_choice) + (Mo_pers[s]*px_motor);
      Q_S1[s,t,2] = omega[s] * Q_MB[s,t,2] + (1-omega[s]) * Q_TD[s,t,2] + (pers[s]*px_choice) + (Mo_pers[s]*px_motor);
      ...
      if (missing_choice[s,t,1]==0) {
        ...
        if(skip_choice[s,t,1]==0) {
        log_lik[s,t,1] = bernoulli_logit_lpmf(choice[s,t,1] | (Q_S1[s,t,2]-Q_S1[s,t,1])*beta_1[s]  );
