...
transformed parameters {
  ...
  for (s in 1:nS){
      if(Group[s]!=0){
        ...
        alpha_normal[s,2] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_sh + alpha_grp[Group[s]] + alpha_shgrp[Group[s]];
        
        beta_1_MF_normal[s,1] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + b1MF_grp[Group[s]];
        beta_1_MF_normal[s,2] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + b1MF_sh + b1MF_grp[Group[s]] + b1MF_shgrp[Group[s]];
        
        beta_1_MB_normal[s,1] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + b1MB_grp[Group[s]];
        beta_1_MB_normal[s,2] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + b1MB_sh + b1MB_grp[Group[s]] + b1MB_shgrp[Group[s]];
        
        beta_2_normal[s,1] = beta_2_m + (beta_2_s*beta_2_raw[s]) + b2_grp[Group[s]];
        beta_2_normal[s,2] = beta_2_m + (beta_2_s*beta_2_raw[s]) + b2_sh + b2_grp[Group[s]] + b2_shgrp[Group[s]];
      }else{
        alpha_normal[s,1] = alpha_m + (alpha_s*alpha_raw[s]);
        alpha_normal[s,2] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_sh;
        
        beta_1_MF_normal[s,1] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]);
        beta_1_MF_normal[s,2] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + b1MF_sh;
        
        beta_1_MB_normal[s,1] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]);
        beta_1_MB_normal[s,2] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + b1MB_sh;
        
        beta_2_normal[s,1] = beta_2_m + (beta_2_s*beta_2_raw[s]);
        beta_2_normal[s,2] = beta_2_m + (beta_2_s*beta_2_raw[s]) + b2_sh;
      }
  }
...
model {
 //define variables
  ...
  alpha_grp ~ normal(0,5);
  alpha_sh ~ normal(0,5);
  alpha_shgrp ~ normal(0,5);
  
  b1MF_grp ~ normal(0,5);
  b1MF_sh ~ normal(0,5);
  b1MF_shgrp ~ normal(0,5);
  
  b1MB_grp ~ normal(0,5);
  b1MB_sh ~ normal(0,5);
  b1MB_shgrp ~ normal(0,5);
