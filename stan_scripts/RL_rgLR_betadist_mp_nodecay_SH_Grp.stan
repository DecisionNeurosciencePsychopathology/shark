...
parameters {
  ...
  real<lower=0> beta_1_MF_sh_s;
  real<lower=0> beta_1_MB_sh_s;
  
  vector[3] alpha_sh_grp;
  vector[3] beta_1_MF_sh_grp;
  vector[3] beta_1_MB_sh_grp;
  
  vector[3] alpha_grp;
  vector[3] beta_1_MF_grp;
  vector[3] beta_1_MB_grp;
  ...
  vector[nS] beta_1_MF_sh_raw;
  vector[nS] beta_1_MB_sh_raw;
...
transformed parameters {
  ...
  vector<lower=0,upper=1>[2] beta_1_MB[nS];
  vector<lower=0>[nS] beta_2;
  
  vector[nS] pers;
  vector[nS] Mo_pers;
  vector[nS] alpha_sh;
  vector[nS] beta_1_MF_sh;
  vector[nS] beta_1_MB_sh;
 
  for (s in 1:nS){
    if(Group[s]==0) {
        alpha_sh[s] = alpha_sh_m + (alpha_sh_s*alpha_sh_raw[s]);
        beta_1_MF_sh[s] = beta_1_MF_sh_m + (beta_1_MF_sh_s*beta_1_MF_sh_raw[s]);
        beta_1_MB_sh[s] = beta_1_MB_sh_m + (beta_1_MB_sh_s*beta_1_MB_sh_raw[s]);
        ...
        beta_1_MF_normal[s,2] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + beta_1_MF_sh[s];
        
        beta_1_MB_normal[s,1] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]);
        beta_1_MB_normal[s,2] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + beta_1_MB_sh[s];
    } else {
        alpha_sh[s] = alpha_sh_m + (alpha_sh_s*alpha_sh_raw[s]) + alpha_sh_grp[Group[s]];
        beta_1_MF_sh[s] = beta_1_MF_sh_m + (beta_1_MF_sh_s*beta_1_MF_sh_raw[s]) + beta_1_MF_sh_grp[Group[s]];
        beta_1_MB_sh[s] = beta_1_MB_sh_m + (beta_1_MB_sh_s*beta_1_MB_sh_raw[s]) + beta_1_MB_sh_grp[Group[s]];
        ...
        beta_1_MF_normal[s,1] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + beta_1_MF_grp[Group[s]];
        beta_1_MF_normal[s,2] = beta_1_MF_m + (beta_1_MF_s*beta_1_MF_raw[s]) + beta_1_MF_grp[Group[s]] + beta_1_MF_sh[s];
        
        beta_1_MB_normal[s,1] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + beta_1_MB_grp[Group[s]];
        beta_1_MB_normal[s,2] = beta_1_MB_m + (beta_1_MB_s*beta_1_MB_raw[s]) + beta_1_MB_grp[Group[s]] + beta_1_MB_sh[s];
    }
  }
  ...
  beta_1_MB = inv_logit(beta_1_MB_normal);
...
model {
 //define variables
  ...
  beta_1_MF_sh_m ~ normal(0,4);
  beta_1_MB_sh_m ~ normal(0,4);
  
  alpha_sh_s ~ cauchy(0,1);
  beta_1_MF_sh_s ~ cauchy(0,1);
  beta_1_MB_sh_s ~ cauchy(0,1);
  
  alpha_sh_raw ~ normal(0,1);
  beta_1_MF_sh_raw ~ normal(0,1);
  beta_1_MB_sh_raw ~ normal(0,1);
  
  alpha_sh_grp ~ normal(0,4);
  beta_1_MF_sh_grp ~ normal(0,4);
  beta_1_MB_sh_grp ~ normal(0,4);
  
  alpha_grp ~ normal(0,4);
  beta_1_MF_grp ~ normal(0,4);
  beta_1_MB_grp ~ normal(0,4);
  ...
  beta_b=1;
    ...
    for (t in 1:nT) {
      ...
      if (missing_choice[s,t,1]==0) {
        ...
        if (missing_choice[s,t,2]==0) {
          ...
          //if (missing_reward[s,t]==0) {
             ...
             Q_TD[(choice[s,t,1] ? 1 : 2)] = decay[s,shark[s,t]+1]*Q_TD[(choice[s,t,1] ? 1 : 2)];
             Q_2[state_2[s,t],(choice[s,t,2] ? 1 : 2)] = decay[s,shark[s,t]+1]*Q_2[state_2[s,t],(choice[s,t,2] ? 1 : 2)];
             unc_state = (state_2[s,t]-1) ? 1 : 2;
             Q_2[unc_state,1] = decay[s,shark[s,t]+1]*Q_2[unc_state,1];
             Q_2[unc_state,2] = decay[s,shark[s,t]+1]*Q_2[unc_state,2];
          ...
          Q_TD[(choice[s,t,1] ? 1 : 2)] = decay[s,shark[s,t]+1]*Q_TD[(choice[s,t,1] ? 1 : 2)];
          Q_2[1,1] = decay[s,shark[s,t]+1]*Q_2[1,1];
          Q_2[1,2] = decay[s,shark[s,t]+1]*Q_2[1,2];
          Q_2[2,1] = decay[s,shark[s,t]+1]*Q_2[2,1];
          Q_2[2,2] = decay[s,shark[s,t]+1]*Q_2[2,2];
      ...
      prev_choice=0;
      prev_motor=0;
      Q_TD[1] = decay[s,shark[s,t]+1]*Q_TD[1];
      Q_TD[2] = decay[s,shark[s,t]+1]*Q_TD[2];
      Q_2[1,1] = decay[s,shark[s,t]+1]*Q_2[1,1];
      Q_2[1,2] = decay[s,shark[s,t]+1]*Q_2[1,2];
      Q_2[2,1] = decay[s,shark[s,t]+1]*Q_2[2,1];
      Q_2[2,2] = decay[s,shark[s,t]+1]*Q_2[2,2];
        ...
        if (missing_choice[s,t,2]==0) {
          ...
          //if (missing_reward[s,t]==0) {
            //prediction errors
             ...
             //update chosen values
             //Q_TD[choice[s,t,1]+1] = Q_TD[choice[s,t,1]+1] + alpha[s,shark[s,t]+1]*(delta_1+lambda[s]*delta_2);
             Q_TD[s,t+1,choice[s,t,1]+1] = Q_TD[s,t,choice[s,t,1]+1] + (alpha[s,shark[s,t]+1]*delta_1[s,t]) + (0.99*delta_2[s,t]);
             ...
             Q_2[s,t+1,state_2[s,t],(choice[s,t,2] ? 1 : 2)] = decay[s,shark[s,t]+1]*Q_2[s,t,state_2[s,t],(choice[s,t,2] ? 1 : 2)];
             unc_state = (state_2[s,t]-1) ? 1 : 2;
             Q_2[s,t+1,unc_state,1] = decay[s,shark[s,t]+1]*Q_2[s,t,unc_state,1];
             Q_2[s,t+1,unc_state,2] = decay[s,shark[s,t]+1]*Q_2[s,t,unc_state,2];
          ...
          Q_2[s,t+1,1,2] = decay[s,shark[s,t]+1]*Q_2[s,t,1,2];
          Q_2[s,t+1,2,1] = decay[s,shark[s,t]+1]*Q_2[s,t,2,1];
          Q_2[s,t+1,2,2] = decay[s,shark[s,t]+1]*Q_2[s,t,2,2];
