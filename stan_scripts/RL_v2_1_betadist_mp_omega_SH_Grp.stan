...
parameters {
  ...
  vector<lower=0,upper=1>[nS] omega_bl;
  
  vector<lower=0>[nS] beta_2;
  vector[nS] pers;
  vector[nS] Mo_pers;
  
  vector[nS] alpha_sh_bl;
  vector[nS] beta1_sh_bl;
  vector[nS] omega_sh_bl;
...
transformed parameters {
  ...
  for (s in 1:nS){
    if(Group[s]==0) {
        ...
        alpha[s,1] = alpha_m + (alpha_s*alpha_raw[s]);
        alpha[s,2] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_sh[s];
        
        beta_1[s,1] = beta_1_m + (beta_1_s*beta_1_raw[s]);
        beta_1[s,2] = beta_1_m + (beta_1_s*beta_1_raw[s]) + beta1_sh[s];
