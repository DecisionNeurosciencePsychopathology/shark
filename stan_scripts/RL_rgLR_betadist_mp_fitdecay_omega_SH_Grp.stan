...
transformed parameters {
  ...
  vector<lower=0,upper=1>[2] decay[nS];
  vector[nS] pers;
  vector[nS] Mo_pers;
  vector[nS] alpha_sh;
  vector[nS] beta1_sh;
  ...
  for (s in 1:nS){
    ...
    for(sk in 1:2){
      decay_normal[s,sk] = decay_m + (decay_s * decay_raw[s]);
    }
  }
