...
transformed parameters {
  ...
  alpha_normal = alpha_m[1] + (alpha_s*alpha_raw) + grp_dep*alpha_m_diff_dep + grp_ide*alpha_m_diff_ide + grp_att*alpha_m_diff_att;
  alpha = inv_logit(alpha_normal);
  beta_1_MF = beta_1_MF_m[1] + (beta_1_MF_s*beta_1_MF_raw) + grp_dep*beta_1_MF_m_diff_dep + grp_ide*beta_1_MF_m_diff_ide + grp_att*beta_1_MF_m_diff_att;
  beta_1_MB = beta_1_MB_m[1] + (beta_1_MB_s*beta_1_MB_raw) + grp_dep*beta_1_MB_m_diff_dep + grp_ide*beta_1_MB_m_diff_ide + grp_att*beta_1_MB_m_diff_att
  beta_2 = beta_2_m[1] + (beta_2_s*beta_2_raw) + grp_dep*beta_2_m_diff_dep + grp_ide*beta_2_m_diff_ide + grp_att*beta_2_m_diff_att;
  
  //Shark Paras
  alpha_normal_shark = alpha_m[2] + (alpha_s*alpha_raw) + grp_dep*alpha_m_diff_dep + grp_ide*alpha_m_diff_ide + grp_att*alpha_m_diff_att;
  alpha_shark = inv_logit(alpha_normal_shark);
  beta_1_MF_shark = beta_1_MF_m[2] + (beta_1_MF_s*beta_1_MF_raw) + grp_dep*beta_1_MF_m_diff_dep + grp_ide*beta_1_MF_m_diff_ide + grp_att*beta_1_MF_m_diff_att;
  beta_1_MB_shark = beta_1_MB_m[2] + (beta_1_MB_s*beta_1_MB_raw) + grp_dep*beta_1_MB_m_diff_dep + grp_ide*beta_1_MB_m_diff_ide + grp_att*beta_1_MB_m_diff_att
  beta_2_shark = beta_2_m[2] + (beta_2_s*beta_2_raw) + grp_dep*beta_2_m_diff_dep + grp_ide*beta_2_m_diff_ide + grp_att*beta_2_m_diff_att;
...
model {
  //define variables
  int prev_choice[nS,nT,2];
  int tran_count[nS,nT];
  int tran_type[nS,nT,2];
  int unc_state[nS,nT];
  real delta_1[nS,nT];
  real delta_2[nS,nT];
  real Q_TD[nS,nT,2];
  ...
  //define priors
  //Means
  alpha_m[1] ~ normal(0,2.5);
  beta_1_MF_m[1] ~ normal(0,5);
  beta_1_MB_m[1] ~ normal(0,5);
  beta_2_m[1] ~ normal(0,5);
  //lambda_m ~ normal(0,2);
  alpha_m[2] ~ normal(0,2.5);
  beta_1_MF_m[2] ~ normal(0,5);
  beta_1_MB_m[2] ~ normal(0,5);
  beta_2_m[2] ~ normal(0,5);
  pers_m ~ normal(0,2.5);
  //SDs
  alpha_s ~ cauchy(0,1);
  ...
  prev_choice=0;
    ...
    for (t in 1:nT) {
      ...
      if (missing_choice[s,t,1]==0) {
        choice[s,t,1] ~ bernoulli_logit(beta_1_MF[s]*(Q_TD[2]-Q_TD[1])+beta_1_MB[s]*(Q_MB[2]-Q_MB[2])+pers[s]*prev_choice);
        ...
        if (missing_choice[s,t,2]==0) {
          ...
          if (missing_reward[s,t]==0) {
             ...
             Q_TD[(choice[s,t,1] ? 2 : 1)] = (1-alpha[s])*Q_TD[(choice[s,t,1] ? 2 : 1)];
             Q_2[state_2[s,t],(choice[s,t,2] ? 2 : 1)] = (1-alpha[s])*Q_2[state_2[s,t],(choice[s,t,2] ? 2 : 1)];
...
generated quantities {
  ...
  for (s in 1:nS) {
    ...
    for (t in 1:nT) {
      if (missing_choice[s,t,1]==0) {
        log_lik[s,t,1] = bernoulli_logit_lpmf(choice[s,t,1] | beta_1_MF[s]*(Q_TD[s,t,2]-Q_TD[s,t,1])+beta_1_MB[s]*(Q_MB[s,t,2]-Q_MB[s,t,2])+pers[s]*prev_choice);
        prev_choice = 2*choice[s,t,1]-1; //1 if choice 2, -1 if choice 1
        
        if (missing_choice[s,t,2]==0) {
          log_lik[s,t,2] = bernoulli_logit_lpmf(choice[s,t,2] | beta_2[s]*(Q_2[s,t,state_2[s,t],2]-Q_2[s,t,state_2[s,t],1]));
          ...
          if (missing_reward[s,t]==0) {
             ...
             //update chosen values
             //Q_TD[choice[s,t,1]+1] = Q_TD[choice[s,t,1]+1] + alpha[s]*(delta_1+lambda[s]*delta_2);
             Q_TD[s,t,choice[s,t,1]+1] = Q_TD[s,t,choice[s,t,1]+1] + alpha[s]*(delta_1[s,t]+delta_2[s,t]);
             Q_2[s,t,state_2[s,t],choice[s,t,2]+1] = Q_2[s,t,state_2[s,t],choice[s,t,2]+1] + alpha[s]*delta_2[s,t];
             Q_MB[s,t,1] = (tran_type[1] > tran_type[2]) ? (.7*fmax(Q_2[s,t,1,1],Q_2[s,t,1,2]) + .3*fmax(Q_2[s,t,2,1],Q_2[s,t,2,2])) : (.3*fmax(Q_2[s,t,1,1],Q_2[s,t,1,2]) + .7*fmax(Q_2[s,t,2,1],Q_2[s,t,2,2]));
             Q_MB[s,t,2] = (tran_type[1] > tran_type[2]) ? (.3*fmax(Q_2[s,t,1,1],Q_2[s,t,1,2]) + .7*fmax(Q_2[s,t,2,1],Q_2[s,t,2,2])) : (.7*fmax(Q_2[s,t,1,1],Q_2[s,t,1,2]) + .3*fmax(Q_2[s,t,2,1],Q_2[s,t,2,2]));
             
             //update unchosen TD & second stage values
             Q_TD[s,t,(choice[s,t,1] ? 2 : 1)] = (1-alpha[s])*Q_TD[s,t,(choice[s,t,1] ? 2 : 1)];
             Q_2[s,t,state_2[s,t],(choice[s,t,2] ? 2 : 1)] = (1-alpha[s])*Q_2[s,t,state_2[s,t],(choice[s,t,2] ? 2 : 1)];
             unc_state = (state_2[s,t]-1) ? 2 : 1;
             Q_2[s,t,unc_state,1] = (1-alpha[s])*Q_2[s,t,unc_state,1];
             Q_2[s,t,unc_state,2] = (1-alpha[s])*Q_2[s,t,unc_state,2];
        ...
        log_lik[s,t,2] = 0;
          ...
          Q_TD[s,t,choice[s,t,1]+1] = Q_TD[s,t,choice[s,t,1]+1] + alpha[s]*delta_1[s,t];
          Q_TD[s,t,(choice[s,t,1] ? 2 : 1)] = (1-alpha[s])*Q_TD[s,t,(choice[s,t,1] ? 2 : 1)];
          Q_2[s,t,1,1] = (1-alpha[s])*Q_2[s,t,1,1];
          Q_2[s,t,1,2] = (1-alpha[s])*Q_2[s,t,1,2];
          Q_2[s,t,2,1] = (1-alpha[s])*Q_2[s,t,2,1];
          Q_2[s,t,2,2] = (1-alpha[s])*Q_2[s,t,2,2];
          //MB update of first stage values based on second stage values, so don't change
      ...
      log_lik[s,t,1] = 0;
      log_lik[s,t,2] = 0;
      Q_TD[s,t,1] = (1-alpha[s])*Q_TD[s,t,1];
      Q_TD[s,t,2] = (1-alpha[s])*Q_TD[s,t,2];
      Q_2[s,t,1,1] = (1-alpha[s])*Q_2[s,t,1,1];
      Q_2[s,t,1,2] = (1-alpha[s])*Q_2[s,t,1,2];
      Q_2[s,t,2,1] = (1-alpha[s])*Q_2[s,t,2,1];
      Q_2[s,t,2,2] = (1-alpha[s])*Q_2[s,t,2,2];
