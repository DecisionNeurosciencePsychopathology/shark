...
parameters {
  ...
  vector[GroupNum+1] alpha_sh;
  vector[GroupNum+1] beta1_sh;
  vector[GroupNum+1] omega_sh;
  
  vector[GroupNum] alpha_grp;
  vector[GroupNum] beta1_grp;
  vector[GroupNum] omega_grp;
...
transformed parameters {
  ...
  for (s in 1:nS){
     if(Group[s]==0) {
        ...
        alpha_normal[s,2] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_sh[(Group[s]+1)];
        
        beta_1_normal[s,1] = beta_1_m + (beta_1_s*beta_1_raw[s]);
        beta_1_normal[s,2] = beta_1_m + (beta_1_s*beta_1_raw[s]) + beta1_sh[(Group[s]+1)];
        
        omega_normal[s,1] = omega_m + (omega_s*omega_raw[s]);
        omega_normal[s,2] = omega_m + (omega_s*omega_raw[s]) + omega_sh[(Group[s]+1)];
     } else {
        alpha_normal[s,1] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_grp[(Group[s])];
        alpha_normal[s,2] = alpha_m + (alpha_s*alpha_raw[s]) + alpha_sh[(Group[s]+1)] + alpha_grp[(Group[s])];
        
        beta_1_normal[s,1] = beta_1_m + (beta_1_s*beta_1_raw[s]) + beta1_grp[(Group[s])];
        beta_1_normal[s,2] = beta_1_m + (beta_1_s*beta_1_raw[s]) + beta1_sh[(Group[s]+1)] + beta1_grp[(Group[s])];
        
        omega_normal[s,1] = omega_m + (omega_s*omega_raw[s]) + omega_grp[(Group[s])];
        omega_normal[s,2] = omega_m + (omega_s*omega_raw[s]) + omega_sh[(Group[s]+1)] + omega_grp[(Group[s])];
     }
  }
  ...
  for (sx in 1:nS) {
    for (r in 1:2){
      beta_1_MF[sx,r] = (1-omega[sx,r])*beta_1[sx,r];
      beta_1_MB[sx,r] = (omega[sx,r])*beta_1[sx,r];
    }
  }
}
...
model {
 //define variables
  ...
  alpha_sh ~ normal(0,2.5);
  beta1_sh ~ normal(0,2.5);
  omega_sh ~ normal(0,2.5);
