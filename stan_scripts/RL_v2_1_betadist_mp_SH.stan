///IN THIS SCRIPT WE TRY THE CENTER VERSION OF THE HIERARCHICAL MODELING
data {
  int<lower=1> nT;
  int<lower=1> nS;
...
parameters {
  ...
  real<lower=0> beta_1_sh_s;
  real<lower=0> omega_sh_s;
  
  vector<lower=0,upper=1>[nS] alpha_bl;
  vector<lower=0>[nS] beta_1_bl;
  vector<lower=0>[nS] omega_bl;
  
  vector<lower=0,upper=1>[nS] alpha_sh;
  vector<lower=0>[nS] beta_1_sh;
  vector<lower=0>[nS] omega_sh;
...
vector[2] omega[nS];
  ...
  for (s in 1:nS) {
      alpha[s,1] = alpha_bl[s];
      beta_1[s,1] = beta_1_bl[s];
      omega[s,1] = omega_bl[s];
      alpha[s,2] = alpha_sh[s];
      beta_1[s,2] = beta_1_sh[s];
      omega[s,2] = omega_sh[s];
  }
}
...
model {
 //define variables
  ...
  beta_1_sh_m ~ normal(0,4);
  omega_sh_m ~ normal(0,4);
  
  alpha_sh_s ~ cauchy(0,1);
  beta_1_sh_s ~ cauchy(0,1);
  omega_sh_s ~ cauchy(0,1);
  
  alpha_sh ~ normal(0,2.5);
  beta_1_sh ~ normal(0,2.5);
  omega_sh ~ normal(0,2.5);
  
  beta_2 ~ normal(beta_2_m,beta_2_s);
  pers ~ normal(pers_m,pers_s);
  Mo_pers ~ normal(Mo_pers_m,Mo_pers_s);


  alpha_bl ~ normal(alpha_m,alpha_s);
  alpha_sh ~ normal(alpha_sh_m,alpha_sh_s);
      
  beta_1_bl ~ normal(beta_1_m,beta_1_s);
  beta_1_sh ~ normal(beta_1_sh_m,beta_1_sh_s);
      
  omega_bl ~ normal(omega_m,omega_s);
  omega_sh ~ normal(omega_sh_m,omega_sh_s);
  
  
  for (s in 1:nS) {
    //set up the parameters with hyperparameters
  ...
  //set initial values
    ...
    //Q_MB[i]=.5;
      ...
      Q_2[2,i]=.5;
   // tran_type[i]=0;
      beta_1_MB[s,i] = (omega[s,i])*beta_1[s,i];
      beta_1_MF[s,i] = (1-omega[s,i])*beta_1[s,i];
    }
    prev_motor=0;
    prev_choice=0;
    alpha_b=1;
