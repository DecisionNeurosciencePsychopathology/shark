...
model {
  ...
  //define priors
  alpha_1_m ~ normal(0,2.5);
  alpha_2_m ~ normal(0,2.5);
  beta_1_m ~ normal(0,5);
  beta_2_m ~ normal(0,5);
  omega_m ~ normal(0,2.5);
  lambda_m ~ normal(0,2.5);
  pers_m ~ normal(0,2.5);
  ...
  prev_choice=0; //should have no effect on predicting choice, like setting this to an NA value
  
    for (t in 1:nT) {
      //use if not missing 1st stage choice
      if (missing_choice[s,t,1]==0) {
        choice[s,t,1] ~ bernoulli_logit(beta_1[s]*(Q_net[2]-Q_net[1])+pers[s]*prev_choice);
        ...
        if (missing_choice[s,t,2]==0) {
          ...
          if (missing_reward[s,t]==0) {
             ...
             //update values
             // Q_TD[choice[s,t,1]+1] = Q_TD[choice[s,t,1]+1] + alpha_1[s]*(delta_1+delta_2);
...
generated quantities {
  ...
  for (s in 1:nS) {
    ...
    for (t in 1:nT) {
      if (missing_choice[s,t,1]==0) {
        log_lik[s,t,1] = bernoulli_logit_lpmf(choice[s,t,1] | beta_1[s]*(Q_net[2]-Q_net[1])+pers[s]*prev_choice);
        prev_choice = 2*choice[s,t,1]-1; //1 if choice 2, -1 if choice 1
        if (missing_choice[s,t,2]==0) {
